...
class Goods{
	...
	public function ajax_get_goods_by_group_status($group_status_id, $page_num,$limit_num){		
		...
		$sql='SELECT goods_id,image,price,group_num,name,group_residue_num,group_status_id,group_begin_date FROM '.config('database.prefix').'goods WHERE status=1 AND group_status_id='.$group_status_id.' ORDER BY goods_id LIMIT '.$limit;
